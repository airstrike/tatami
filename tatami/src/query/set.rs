...
pub enum Set {
    ...
    /// The immediate children of `of`.
    Children {
        /// The parent member.
        of: MemberRef,
    },
    /// All descendants of `of` down to `to_level` (inclusive).
    Descendants {
        /// The ancestor member.
        of: MemberRef,
        /// The target depth.
...
mod tests {
    ...
    fn union_roundtrips() {
        let s = Set::Union {
            left: Box::new(Set::Children {
                of: mr("Geography", "EMEA"),
            }),
            right: Box::new(Set::Children {
                of: mr("Geography", "APAC"),
            }),
        };
        let json = serde_json::to_string(&s).expect("serialize");
        let back: Set = serde_json::from_str(&json).expect("deserialize");
